...
CliniClarity is not a diagnostic engine; it is a specialized research librarian engineered to help patients navigate their complex medical journey.
...
* **Unverified Sources:** When turning to search engines, patients encounter a mix of "Dr.Google" anecdotes, unverified blogs, and marketing that lacks clinical validity.
...
2. **Interactive Research Assistant (ReAct):** Once the summary is provided, the system transitions into an autonomous agent to handle follow-up questions (Ex: _"What does this specific white blood cell count mean for my diagnosis?"_)
     * First, the agent queries the OpenSearch vector store to retrieve specific context from the user's uploaded medical report.
     * The LLM then follows a Thinking -> Action -> Observation loop to determine if the internal record provides enough information or if external validation is required.
     ...
     * The result from PubMed is passed back to the LLM as part of the observation, ensuring the final answer is a synthesis of patient data and peer-reviewed literature.
